...
First prepare the separate official `website-chat` Hermes profile at `127.0.0.1:8642`: GLM-5.3, high reasoning, Asia/Shanghai time, four turns, a 120-second run budget, native website memory writes and automatic background review disabled, and eight audited read-only tools (current time, native web, and the `website-readonly` memory/knowledge adapter). Use the canonical identity and website overlay in `services/hermes-chat/assistant-SOUL.md` and `services/hermes-chat/website-profile.yaml`. Back up the live profile, then merge the overlay keys into it; never replace the whole config with the overlay. Replace each listed path with the overlay value, including its lists and complete `mcp_servers` map. Preserve its existing `zai` provider structure, `https://open.bigmodel.cn/api/paas/v4` base URL, `glm-5.3` model, secrets, disabled skills, native session/log settings, and all unrelated fields. Install its gateway through `hermes -p website-chat gateway install` and enable/start its official service. Install the native plugin only in the `website-chat` and `wechat-public` profiles as described in [the operation audit and installation guide](../services/hermes-chat/READONLY.md); registration rejects default, owner, custom, and other profiles. Other profiles' curated native memory files remain intentionally readable through the adapter. The bridge sends the configured profile's `hermes` model alias.
...
Before restarting the website gateway, follow the fixture, missing-plugin and installed-profile checks in [READONLY.md](../services/hermes-chat/READONLY.md). Confirm `timezone: Asia/Shanghai`, `platform_toolsets.api_server: [web, website_readonly, no_mcp]`, empty `cli`/`weixin` lists, `plugins.enabled: [website-readonly]`, `mcp_servers: {}`, `context.engine: compressor`, `agent.disabled_toolsets: [kanban, context_engine]`, false native memory flags and `auxiliary.background_review.enabled: false`. Actual runtime definitions must be exactly `current_datetime`, `web_search`, `web_extract`, `shared_memory_list`, `shared_memory_read`, `knowledge_list`, `knowledge_read`, and `knowledge_search`. Check through the installed Hermes Python and native plugin loader; an absent plugin or any extra tool is a deployment failure. Prompt previews are limited by Hermes's native 4,000-character section cap; remaining memory uses pagination. No broad file/skills/browser group is enabled. Re-run the source-scoped audit after upgrades.

After restarting only the `website-chat` gateway, exercise the clock and web tools through the authenticated loopback API without displaying the bearer token. Verify `current_datetime` matches the synchronized host clock and Asia/Shanghai timezone, a targeted public search and public homepage extraction succeed, responses identify keyless/free handling with no Tavily credential present, and extraction of loopback and cloud-metadata targets is refused before any provider call. The observed v0.21.1 baseline was about 3.5 seconds for native search and 3.2 seconds for native homepage extraction; these are audit observations, not guarantees of latency or search completeness. Then validate the bridge proxy and public service as above. Do not add a paid web service, API key, database, second backend, or Hermes source patch. The small native-plugin adapter is the reviewed extension; it does not replace Hermes engines or session storage.
...
Prepare `wechat-public` with the same canonical `services/hermes-chat/assistant-SOUL.md` and the non-secret `services/hermes-chat/wechat-public-profile.yaml` overlay. It exposes the same eight reviewed read-only functions on `platform_toolsets.weixin` while keeping `api_server` and `cli` empty, native memory writes/review off, and four turns/120 seconds. Preserve all model/provider credentials, Weixin identity IDs, login/transport settings, session/log settings, and unrelated fields; the overlay does not supply them. Run the Weixin fixture and installed-profile verifier forms from [READONLY.md](../services/hermes-chat/READONLY.md), then prove an unknown user resolves to this profile and cannot call writers, terminal, native memory mutation, raw session search, or discovery bridges. Administrative command gating still uses the actual QR owner's ID in `allow_admin_from`; an empty list disables that gate. Full-tool routing for a verified owner is a separate rollout and is not asserted active by this document. These are profile configuration changes, not Hermes source patches.
