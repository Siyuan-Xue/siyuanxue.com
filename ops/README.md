...
First prepare the separate official `website-chat` Hermes profile at `127.0.0.1:8642`: GLM-5.3, high reasoning, four turns, a 120-second run budget, native website memory writes and automatic background review disabled, and seven audited read-only tools (native web plus the `website-readonly` memory/knowledge adapter). Use the reviewed persona and partial overlay in `services/hermes-chat/website-SOUL.md` and `services/hermes-chat/website-profile.yaml`. Back up the live profile, then merge the overlay keys into it; never replace the whole config with the overlay. Replace each listed path with the overlay value, including its lists and complete `mcp_servers` map. Preserve its existing `zai` provider structure, `https://open.bigmodel.cn/api/paas/v4` base URL, `glm-5.3` model, secrets, disabled skills, native session/log settings, and all unrelated fields. Install its gateway through `hermes -p website-chat gateway install` and enable/start its official service. Install the native plugin only in the website profile as described in [the operation audit and installation guide](../services/hermes-chat/READONLY.md). Other profiles remain untouched; their curated native memory files are intentionally readable through the adapter. The bridge sends the configured profile's `hermes` model alias.
...
Before restarting the website gateway, follow the fixture, missing-plugin and installed-profile checks in [READONLY.md](../services/hermes-chat/READONLY.md). Confirm `platform_toolsets.api_server: [web, website_readonly, no_mcp]`, empty `cli`/`weixin` lists, `plugins.enabled: [website-readonly]`, `mcp_servers: {}`, `context.engine: compressor`, `agent.disabled_toolsets: [kanban, context_engine]`, false native memory flags and `auxiliary.background_review.enabled: false`. Actual runtime definitions must be exactly `web_search`, `web_extract`, `shared_memory_list`, `shared_memory_read`, `knowledge_list`, `knowledge_read`, and `knowledge_search`. Check through the installed Hermes Python and native plugin loader; an absent plugin or any extra tool is a deployment failure. Prompt previews are limited by Hermes's native 4,000-character section cap; remaining memory uses pagination. No broad file/skills/browser group is enabled. Re-run the source-scoped audit after upgrades.
