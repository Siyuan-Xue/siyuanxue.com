...
export function validMessages(body) {
  ...
  return (
    ...
    m.length % 2 === 1 &&
    m.reduce(
      (n, x) => n + (typeof x?.content === 'string' ? x.content.length : 16001),
      0,
    ) <= 16000 &&
    m.every(
      (x, i) =>
...
export function createChatServer({
  ...
  const server = createServer(async (req, res) => {
    ...
    if (!client) {
      if (clients.size >= 10000) return json(429, 'rate_limited');
      client = { times: [], active: 0, last: now };
      clients.set(ip, client);
    }
    client.times = client.times.filter((t) => now - t < 60000);
    client.last = now;
    ...
    const timer = setTimeout(() => {
      controller.abort();
      if (!res.headersSent) json(502, 'upstream_unavailable');
    ...
    try {
      ...
      for await (const chunk of upstream.body) {
        ...
        while ((match = /\r?\n\r?\n/.exec(buffer))) {
          ...
          if (event.error || /^event:\s*error\s*$/m.test(frame))
