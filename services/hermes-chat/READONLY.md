...
The active public platform's final model-visible definitions, after native tool assembly, must contain exactly these eight names:
...
| Native operations audited | Public-profile decision and reason |
|---|---|
| `current_datetime` (plugin wrapper around `hermes_time.now`) | Enabled in both public profiles. Each call obtains a new aware datetime from Hermes and returns ISO local/UTC time, IANA timezone, integer epoch seconds, and weekday. The wrapper has no arguments and performs no direct filesystem, subprocess, or network access; native `hermes_time` may read the profile's timezone configuration during initialization. An unavailable or unaware clock returns an error instead of a fabricated time. |
...
| `x_search` (`tools/x_search_tool.py`) | Public X search is a reader, but needs xAI credentials and its native resolver can refresh OAuth. It is not part of either reviewed public-profile overlay. |
| `ha_list_entities`, `ha_get_state`, `ha_list_services` (`tools/homeassistant_tool.py`) | GET-based reader operations, but depend on a configured Home Assistant endpoint/token and can reveal private home state. They are not part of either public-profile overlay. `ha_call_service` is a distinct mutating POST; the mixed `homeassistant` group must never be enabled wholesale. |
| Feishu document/comment list operations | Native read operations require Feishu client/context. They are not part of either public-profile overlay. Comment reply/add and other messaging operations mutate external state; not enabled. |
| Spotify, Discord, Yuanbao, remote connections, MCP | Mixed operation/account surfaces or require separately reviewed accounts/providers. No full group/connector is enabled. Arbitrary MCP “read-only” annotations are not authorization or an audit. Additional operations require a fresh implementation and credential review; this policy makes no universal third-party-provider support claim. |
...
This is an operation review of the installed native catalog, not a name-prefix, `readOnlyHint`, or parallel-safety classifier. Only existing, useful and safely scoped readers are exposed. The six plugin functions are a custom **Hermes extension adapter**, not built-in native functions. Hermes owns discovery, schemas, dispatch, prompt sections, sessions and logs.

## Shared memory and knowledge boundaries

The default root is `/home/ubuntu/.hermes`. `shared_memory_list` discovers the root plus every current native directory under `profiles/`, including profiles created later. `shared_memory_read` accepts only `memories/MEMORY.md` and `memories/USER.md` for the selected profile. Nothing is copied or written. Both public overlays set `memory_enabled` and `user_profile_enabled` false and `auxiliary.background_review.enabled` false: native public-profile memory writes/review are off, while plugin shared reads are on. Missing files are normal and do not get created by a read.
...
Copy only `website-readonly/__init__.py` and `plugin.yaml` into `/home/ubuntu/.hermes/profiles/website-chat/plugins/website-readonly/` and `/home/ubuntu/.hermes/profiles/wechat-public/plugins/website-readonly/`, owned by each gateway's existing OS account. Do not install it in the default, owner, custom, or any other profile; registration also refuses those active profiles. Back up each public profile's config/SOUL first, merge its reviewed overlay with exact replacement of the listed lists/maps, and copy the canonical `assistant-SOUL.md` to `SOUL.md`. Preserve provider/model/secrets, identity IDs, login/transport settings, and unrelated fields. Ensure any existing `plugins.disabled` does not deny `website-readonly`; the verifier treats a missing plugin as a failed installation.

`plugins.enabled: [website-readonly]`, `mcp_servers: {}`, and `agent.disabled_toolsets: [kanban, context_engine]` are deliberate. Website uses `platform_toolsets.api_server: [web, website_readonly, no_mcp]` with empty `cli`/`weixin`; Weixin public uses `platform_toolsets.weixin: [web, website_readonly, no_mcp]` with empty `api_server`/`cli`. `web` is an explicit known group even if the plugin is absent, preventing a full-composite fallback. With a missing plugin only the two safe native web schemas remain; deployment must stop because the required eight-schema audit fails. The hook is an extra veto, not the primary boundary: the effective model schemas already contain only readers. No native file/skills group or built-in override is registered.

Set `tools.tool_search.enabled: "off"` with the quoted string exactly as shown in both overlays. At this Hermes commit, the default `auto` defers plugin readers behind `tool_search`, `tool_describe`, and `tool_call`; those bridge tools are outside the public policy and remain denied. Disabling deferral exposes the eight reviewed schemas directly. Auditing only `get_tool_definitions(..., skip_tool_search_assembly=True)` misses this integration failure: the verifier checks both that raw catalog and the final `get_tool_definitions(...)` result. Its native fixture also reproduces the legacy `auto` assembly and confirms that the bridges remain vetoed, without changing the fixture configuration or expanding permissions.
...
python services/hermes-chat/verify-readonly.py --hermes-source /path/to/hermes --fixture --profile wechat-public
...
HERMES_HOME=/home/ubuntu/.hermes/profiles/wechat-public python services/hermes-chat/verify-readonly.py --hermes-source /path/to/hermes --profile wechat-public
```

Fixture mode defaults to `website-chat`/`api_server`; `--profile wechat-public` selects the matching overlay and `weixin` platform. It copies the plugin/config into a disposable native profile and verifies actual registration, raw and final schemas, timezone and native clock consistency, prompt integration, fresh/new-profile reads, credential masking, and write/unknown-tool vetoes. No model call or external request is made. Missing-plugin mode checks the exact two-web fallback for either fixture. Installed-profile mode does not change source memories or config and prints only tool names/status; Hermes may update its normal registration cache. Run `HERMES_SOURCE=/path/to/hermes python -m unittest discover -s tests -p test_hermes_readonly.py` for additional real-filesystem boundary tests and native redaction coverage. Without `HERMES_SOURCE`, that single redactor test is explicitly skipped; the remaining suite uses only stdlib.
...
Only after each installed-profile audit passes, restart the corresponding gateway and check real structured clock/search tool use, public web SSRF denial, fresh session memory visibility, correct website/Weixin profile routing, and absence of public management endpoints. Leave native session/log retention and owner-profile permissions separate. Re-audit this source-scoped policy after any Hermes upgrade.
