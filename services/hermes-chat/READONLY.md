...
|---|---|
| `current_datetime` (plugin wrapper around `hermes_time.now`) | Enabled in both public profiles. Each call obtains a new aware datetime from Hermes and returns ISO local/UTC time, IANA timezone, integer epoch seconds, and weekday. It has no arguments and performs no subprocess, network, or file access. An unavailable or unaware clock returns an error instead of a fabricated time. |
